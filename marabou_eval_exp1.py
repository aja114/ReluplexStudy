...
def set_constraints(network, inputVars, outputVars, target):
	...
	if target == "square":
		# Set output interval
		network.setUpperBound(outputVars, 0)
	...
	if target == "inverse":
	   	# Set output equality as a very tight lower and upper bound
		network.setLowerBound(outputVars, -0.00001)
		network.setUpperBound(outputVars, 0.00001)
...
for target in targets:
	path = os.path.join('models_exp1', f'model_{target}')
...
results.to_csv('results_exp1.csv')
